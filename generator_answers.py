...
RET_RESULTS_FILE = "ret_results_filtered_Hybrid_15.json"
SPLIT_DOCUMENTS_FILE = "split_documents_with_references.json"  
START_INDEX = 301
END_INDEX = 400
BATCH_NUMBER = 1  # Increment this for each new batch run
...
def format_chunk_for_context(doc: Dict[str, Any]) -> str:
    ...
    # Use modified_text_2 instead of text
...
def generate_answers(
    ...
    for i, item in enumerate(questions_to_process):
        try:
            ...
            # Add a delay before processing each question (except the first one)
            if i > 0:
            ...
            chunks_content = []
            chunk_texts_preview = []  # Store text previews for output
            ...
            for chunk in chunks_with_metadata:
                ...
                if parent_id and chunk_number:
                    # Get the specific document matching both parent_id and chunk_number
            ...
            **CONTEXT:**  
            ...
            results.append({
                ...
                "batch_number": BATCH_NUMBER,                # Track the batch number
                "question": question,
    ...
    try:
        ...
        if os.path.exists(file_path):
            try:
                ...
                # If we can't load existing results, we'll just start fresh
                existing_results = []
...
def main():
    ...
    try:
        ...
        # Save results
        results_file = f"gen_answers_{CHUNK_TYPE}_{os.path.basename(RET_RESULTS_FILE)}"
