...
CONFIDENCE_INTERVAL = 95  # 95% confidence interval
COUNT_FOLDER = "count"
OUTPUT_FOLDER = "bootstrap_results"

# *** Define question ID range variables here ***
START_QUESTION_ID = 1  # For Q001
END_QUESTION_ID = 400  # For Q100
...
def load_data_from_count_folder(start_qid, end_qid):
    ...
    # Filter data based on question ID range
    for item in all_data:
        ...
        if qid.startswith("Q"):
            try:
                ...
                # Skip items with invalid question ID format
                continue
...
def aggregate_scores(data):
    ...
    for (qid, method, k, version), item in question_configs.items():
        # Step 1: Average across evaluator models for each metric
        metric_averages = {}
        ...
        for metric in ["faithfulness", "completeness", "relevancy"]:
            if metric in item:
                ...
                # Calculate average score for this metric across all evaluator models
                if model_scores:
                    metric_averages[metric] = sum(model_scores) / len(model_scores)
        
        # Step 2: Average across metrics - Both arithmetic and geometric mean
        if metric_averages:
...
def generate_visualizations(df, config_results, factor_results):
    """Generate visualizations for the bootstrap results."""
    
    print(f"\nGenerating visualizations in {OUTPUT_FOLDER}...")
    
    # 1. Overall Rankings bar chart - Arithmetic Mean
    plt.figure(figsize=(12, 8))
    
    # Prepare data for plotting
    configs = [f"{r['method']}, k={r['k']}, {r['version']}" for r in config_results["arithmetic"]]
    means = [r["mean"] for r in config_results["arithmetic"]]
    errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in config_results["arithmetic"]]
    
    # Create bar chart
    plt.bar(configs, means, yerr=errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("Configuration Rankings by Arithmetic Mean Score (Stratified Bootstrap)")
    plt.ylabel("Arithmetic Mean Score")
    plt.xticks(rotation=45, ha="right")
    plt.ylim(1.5, 3.0)  # Adjust based on your score range
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "overall_rankings_arithmetic.png"))
    plt.close()
    
    # 2. Overall Rankings bar chart - Geometric Mean
    plt.figure(figsize=(12, 8))
    
    # Prepare data for plotting
    configs = [f"{r['method']}, k={r['k']}, {r['version']}" for r in config_results["geometric"]]
    means = [r["mean"] for r in config_results["geometric"]]
    errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in config_results["geometric"]]
    
    # Create bar chart
    plt.bar(configs, means, yerr=errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("Configuration Rankings by Geometric Mean Score (Stratified Bootstrap)")
    plt.ylabel("Geometric Mean Score")
    plt.xticks(rotation=45, ha="right")
    plt.ylim(1.5, 3.0)  # Adjust based on your score range
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "overall_rankings_geometric.png"))
    plt.close()
    
    # 3. Method comparison - Arithmetic Mean
    plt.figure(figsize=(8, 6))
    methods = [r["method"] for r in factor_results["arithmetic"]["method"]]
    method_means = [r["mean"] for r in factor_results["arithmetic"]["method"]]
    method_errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in factor_results["arithmetic"]["method"]]
    
    plt.bar(methods, method_means, yerr=method_errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("Retrieval Method Comparison - Arithmetic Mean (Stratified Bootstrap)")
    plt.ylabel("Arithmetic Mean Score")
    plt.ylim(1.5, 3.0)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "method_comparison_arithmetic.png"))
    plt.close()
    
    # 4. Method comparison - Geometric Mean
    plt.figure(figsize=(8, 6))
    methods = [r["method"] for r in factor_results["geometric"]["method"]]
    method_means = [r["mean"] for r in factor_results["geometric"]["method"]]
    method_errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in factor_results["geometric"]["method"]]
    
    plt.bar(methods, method_means, yerr=method_errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("Retrieval Method Comparison - Geometric Mean (Stratified Bootstrap)")
    plt.ylabel("Geometric Mean Score")
    plt.ylim(1.5, 3.0)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "method_comparison_geometric.png"))
    plt.close()
    
    # 5. K value comparison - Arithmetic Mean
    plt.figure(figsize=(8, 6))
    k_values = [f"k={r['k']}" for r in factor_results["arithmetic"]["k"]]
    k_means = [r["mean"] for r in factor_results["arithmetic"]["k"]]
    k_errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in factor_results["arithmetic"]["k"]]
    
    plt.bar(k_values, k_means, yerr=k_errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("K Value Comparison - Arithmetic Mean (Stratified Bootstrap)")
    plt.ylabel("Arithmetic Mean Score")
    plt.ylim(1.5, 3.0)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "k_comparison_arithmetic.png"))
    plt.close()
    
    # 6. K value comparison - Geometric Mean
    plt.figure(figsize=(8, 6))
    k_values = [f"k={r['k']}" for r in factor_results["geometric"]["k"]]
    k_means = [r["mean"] for r in factor_results["geometric"]["k"]]
    k_errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in factor_results["geometric"]["k"]]
    
    plt.bar(k_values, k_means, yerr=k_errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("K Value Comparison - Geometric Mean (Stratified Bootstrap)")
    plt.ylabel("Geometric Mean Score")
    plt.ylim(1.5, 3.0)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "k_comparison_geometric.png"))
    plt.close()
    
    # 7. Version comparison - Arithmetic Mean
    plt.figure(figsize=(8, 6))
    versions = [r["version"] for r in factor_results["arithmetic"]["version"]]
    version_means = [r["mean"] for r in factor_results["arithmetic"]["version"]]
    version_errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in factor_results["arithmetic"]["version"]]
    
    plt.bar(versions, version_means, yerr=version_errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("Version Comparison - Arithmetic Mean (Stratified Bootstrap)")
    plt.ylabel("Arithmetic Mean Score")
    plt.ylim(1.5, 3.0)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "version_comparison_arithmetic.png"))
    plt.close()
    
    # 8. Version comparison - Geometric Mean
    plt.figure(figsize=(8, 6))
    versions = [r["version"] for r in factor_results["geometric"]["version"]]
    version_means = [r["mean"] for r in factor_results["geometric"]["version"]]
    version_errors = [(r["upper_ci"] - r["lower_ci"]) / 2 for r in factor_results["geometric"]["version"]]
    
    plt.bar(versions, version_means, yerr=version_errors, capsize=5)
    plt.axhline(y=3.0, color='r', linestyle='-', label='100%')  # Add horizontal line at 100% (3.0)
    plt.title("Version Comparison - Geometric Mean (Stratified Bootstrap)")
    plt.ylabel("Geometric Mean Score")
    plt.ylim(1.5, 3.0)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "version_comparison_geometric.png"))
    plt.close()
    
    # 9. Heatmap of method, k, version combinations - Arithmetic Mean
    pivot_arith = df.pivot_table(
        index=["method", "k"], 
        columns=["version"], 
        values="arith_avg_score", 
        aggfunc="mean"
    )
    
    plt.figure(figsize=(10, 6))
    sns.heatmap(pivot_arith, annot=True, cmap="viridis", vmin=1.5, vmax=3.0, cbar_kws={'label': 'Arithmetic Mean Score'})
    plt.title("Configuration Performance Heatmap - Arithmetic Mean")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "configuration_heatmap_arithmetic.png"))
    plt.close()
    
    # 10. Heatmap of method, k, version combinations - Geometric Mean
    pivot_geo = df.pivot_table(
        index=["method", "k"], 
        columns=["version"], 
        values="geo_avg_score", 
        aggfunc="mean"
    )
    
    plt.figure(figsize=(10, 6))
    sns.heatmap(pivot_geo, annot=True, cmap="viridis", vmin=1.5, vmax=3.0, cbar_kws={'label': 'Geometric Mean Score'})
    plt.title("Configuration Performance Heatmap - Geometric Mean")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, "configuration_heatmap_geometric.png"))
    plt.close()
...
def main():
    ...
    generate_visualizations(df, config_results, factor_results)
    ...
    print(f"\nCheck {OUTPUT_FOLDER} folder for detailed visualizations and report.")
    print(f"Report file: bootstrap_report_Q{START_QUESTION_ID:03d}_Q{END_QUESTION_ID:03d}.md")
