...
#         # ONLY extract parent_id and references from metadata
...
#     """Main RAG pipeline that respects UI controls"""
#     start_time = datetime.now()
    
#     # Get LLM
#     llm = get_llm()
    
#     # Use your single set of embedding files
#     embeddings_path = "split_documents.npy"
#     texts_path = "split_documents.json"
...
#         retriever = SparseRetriever(texts_path, k)
...
#         # Use the existing filter_documents_by_llm function
...
# def display_answer_with_links(answer):
...
#         button_key = f"link_chunk_{chunk_num}_{i}"
...
#         options=["Off", "On"],
#         index=0  # Default to first option (Off)
#     )
...
#                 display_answer_with_links(message["answer"])
#             else:
...
#                 display_answer_with_links(result["answer"])
...
def run_single_retrieval(question, retriever_type, k, alpha, use_hyde, use_llm_filtering, use_cosine_filtering, cosine_threshold, llm):
    ...
    if retriever_type == "Dense_Retriever":
        ...
        retriever = SparseRetriever(texts_path="split_documents.json", sparse_k=k)
    elif retriever_type == "Hybrid_Retriever":
        retriever = HybridRetriever(alpha=alpha, top_k=k)
    else:  # Dense with HyDE
        retriever = DenseRetriever(k=k, llm=llm)
